...
| **Cantidad y U.M.** | 🔢 Se muestran en la **U.M. del documento** (15 P48), con las unidades sueltas debajo (720 UND). ⚠️ **En la réplica los nombres están cruzados**: `cantidad_inv` es la cantidad en `um`, y `cantidad` las unidades sueltas — confirmado por María José en SIESA el 2026-09-14 con CEA-00348223 (COLCAFE, $142.890 ÷ 15 = $9.526 = cotización P48). Los costos van por U.M. —comparables con la cotización— y por unidad suelta. El **reparto** de ajustes sigue en unidades sueltas: es lo único sumable entre presentaciones. **No "arreglar" el nombre de la columna**. Excepción medida: con **paquetes fraccionarios** `cantidad_inv` viene **redondeada** (3 unidades en P2 = 1,5 y dice 2; el costo por P2 salía 30 % más bajo). En esos casos se usa unidades ÷ factor de la U.M.: 9 de 34.908 renglones P, y la regla coincide con el precio unitario de SIESA en los 9 |
| **Filtros en los encabezados** | 📊 Pedido de María José, 2026-09-14. Cada encabezado ordena (menor/mayor, A–Z, fecha) y filtra (valores con buscador y conteo; mínimo/máximo; desde/hasta), como Excel. Lógica en `utils/tablaColumnas.js` (+ tests) y panel en `components/FiltroColumna.jsx`, reutilizables. El Excel exporta **lo que se ve**. Hoy solo en Diferencias de costo; pasarlo a Maestro, Bandeja y Cotizaciones es definir sus columnas |
| **Qué ve el proveedor** | 🔒 **Solo lo de MAYOR costo (CAE).** Johan, 2026-09-14: *"por menor valor no mostrar a los proveedores NUNCA. A compras sí debe mostrarle todo"*. Tampoco las facturas mixtas (CAS y CAE): su costo real trae el CAS adentro. Se filtra **en el backend** (`visibleParaProveedor` en `services/seguimientoDiferencias.js`, con tests) para que las filas ni salgan del servidor; el front lo repite en `useDiferenciasCosto` solo para no depender del despliegue. El aviso de Inicio dice "por encima del valor de la orden", y los filtros "Menor costo" y "Revisar" son solo de compras. ✅ **Verificado en producción** (commit `ae4f9e7`, 2026-09-14): la respuesta cruda de la API para Altipal 006 pasó de 153 filas (66 mayor + 87 menor) a **66, todas de mayor costo** |
